...
// Client-side homepage with animations and lead intake form

import React, { useState, useEffect, useRef } from "react";
import Link from "next/link";
import { ArrowRight, Menu, X, ShieldCheck, Clock, MapPin } from "lucide-react";
...
const FadeUp = ({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) => {
  ...
  return (
    <div
      ref={ref}
      className={`transition-all duration-[1000ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} ${className}`}
...
type CityOption = { name: string; slug: string; state: { slug: string } };

type Props = {
  services: ServiceOption[];
  featuredCities: CityOption[];
};

export function HomePageClient({ services, featuredCities }: Props) {
    ...
    <div className="min-h-screen bg-[#F7F7F9] font-sans text-[#1d1d1f] selection:bg-blue-200 selection:text-black overflow-x-hidden">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled || isMobileMenuOpen ? "bg-white/90 backdrop-blur-xl border-b border-gray-200/50 shadow-sm py-4" : "bg-transparent py-6"}`}>
        <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-[14px] font-semibold tracking-wide">
            <a href="#quote-form" className="text-gray-600 hover:text-black transition-colors">Get Free Quotes</a>
            <a href="#for-detailers" className="text-gray-600 hover:text-black transition-colors">For Pros</a>
      ...
      <div className={`fixed inset-0 bg-white z-40 transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="pt-28 px-6 flex flex-col gap-8 text-2xl font-bold tracking-tight">
          <a href="#quote-form" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-900 border-b border-gray-100 pb-4">Get Free Quotes</a>
          <a href="#for-detailers" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-900 border-b border-gray-100 pb-4">For Pros</a>
          <Link href="/login" className="bg-black text-white w-full py-4 rounded-full mt-4 text-xl shadow-lg text-center">Sign In</Link>
        </div>
      </div>

      {/* Hero */}
      <section id="quote-form" className="relative pt-36 md:pt-48 pb-16 md:pb-24 px-6 max-w-[1200px] mx-auto text-center z-10">
        <FadeUp>
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-[88px] font-bold tracking-tighter leading-[1.05] mb-6">
            The perfect finish.<br /><span className="text-gray-400">Zero friction.</span>
          </h1>
        </FadeUp>
        <FadeUp delay={100}>
        ...
        <FadeUp delay={200} className="w-full">
          <LeadIntakeForm services={services} />
        </FadeUp>
      </section>
      ...
      <section className="px-4 md:px-8 max-w-[1600px] mx-auto mb-20 md:mb-32">
        <FadeUp delay={300}>
          <div className="w-full h-[40vh] md:h-[60vh] lg:h-[70vh] rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-gray-900 relative shadow-2xl">
            <img
              src="https://images.unsplash.com/photo-1569263979104-865ab7cd8d13?auto=format&fit=crop&w=2400&q=80"
              alt="Luxury Yacht"
              className="absolute inset-0 w-full h-full object-cover opacity-90 scale-105 hover:scale-100 transition-transform duration-[10s] ease-out"
            />
          </div>
        </FadeUp>
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-10 md:gap-16">
            {[
              ...
              <FadeUp key={i} delay={i * 100} className="flex flex-col items-center text-center">
                <div className="w-16 h-16 rounded-2xl bg-gray-50 flex items-center justify-center text-black mb-6 shadow-sm border border-gray-100">
                  {feature.icon}
                </div>
      ...
      {featuredCities.length > 0 && (
        <section className="py-10 md:py-20 px-6 max-w-[1400px] mx-auto mb-10 md:mb-20">
            ...
            <div className="flex flex-wrap gap-3">
              {featuredCities.map((city) => (
                <span
                  ...
                  className="px-5 py-2.5 rounded-full bg-white border border-gray-200 text-sm font-semibold text-gray-700 shadow-sm"
                >
                  {city.name}
                </span>
              ))}
          ...
          <Link href="/florida" className="w-full md:hidden mt-6 bg-white border-2 border-gray-200 text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2">
            Explore all regions <ArrowRight size={18} />
          </Link>
        </section>
      )}
      ...
      <section id="for-detailers" className="bg-black text-white py-20 md:py-32 px-6 overflow-hidden">
        <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row items-center justify-between gap-16 md:gap-20">
          <div className="lg:w-1/2 text-center lg:text-left">
            <FadeUp>
              <div className="inline-block px-4 py-1.5 rounded-full bg-white/10 text-white text-xs md:text-sm font-bold tracking-widest uppercase mb-6 md:mb-8 border border-white/20">
                For Detailing Professionals
              </div>
              <h2 className="text-4xl sm:text-5xl md:text-[72px] font-bold tracking-tighter mb-6 md:mb-8 leading-[1.1] md:leading-[1.05]">
            ...
            <FadeUp delay={200}>
              <div className="bg-[#1c1c1e]/80 backdrop-blur-2xl rounded-2xl md:rounded-[2.5rem] p-6 md:p-10 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] md:shadow-[0_30px_80px_rgba(0,0,0,0.8)] transform md:-rotate-2 md:hover:rotate-0 transition-transform duration-500 ease-out">
                <div className="flex justify-between items-center mb-6 md:mb-10 border-b border-white/10 pb-4 md:pb-6">
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs md:text-sm font-medium text-green-400 flex items-center gap-1">
                ...
                <div className="bg-white/5 rounded-xl md:rounded-3xl p-5 md:p-6 border border-white/10 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 md:w-1.5 h-full bg-[#ff385c]" />
                  ...
                  <div className="flex items-center gap-2 md:gap-3 text-sm md:text-base text-gray-300 mb-6 md:mb-8 bg-black/40 w-fit px-3 md:px-4 py-2 md:py-2.5 rounded-lg md:rounded-xl border border-white/5">
                    ...
                    <span className="font-semibold truncate">St. Petersburg, FL</span>
                  </div>
                  <button className="w-full bg-white hover:bg-gray-200 text-black font-bold text-base md:text-lg py-3 md:py-4 rounded-lg md:rounded-xl transition-colors flex justify-between items-center px-4 md:px-6 group">
                    ...
                    <span className="flex items-center gap-1 md:gap-2 bg-black/5 px-2 md:px-3 py-1 rounded-md md:rounded-lg">
                      $20 <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
                    </span>
                  </button>
                </div>
              </div>
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12 mb-16 md:mb-20">
          ...
          <div>
            ...
            <ul className="space-y-3 md:space-y-4 text-gray-500 font-medium text-sm md:text-base">
              <li><a href="#quote-form" className="hover:text-black transition-colors">Get Free Quotes</a></li>
